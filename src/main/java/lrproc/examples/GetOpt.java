...
public class GetOpt
{
  ...
  public int processArg(String arg, int n)
  {
    ...
    {
      if (optErr) System.err.println("processArg cannot process " + arg + " as an integer");
      return n;
    }
    return value;
  }
  ...
  public int tryArg(int k, int n)
  {
    ...
    {
      if (optErr) System.err.println("tryArg: no theArgs[" + k + "]");
      return n;
    }
    return value;
  }
  ...
  public long processArg(String arg, long n)
  {
    ...
    {
      if (optErr) System.err.println("processArg cannot process " + arg + " as a long");
      return n;
    }
    return value;
  }
  ...
  public double processArg(String arg, double d)
  {
    ...
    {
      if (optErr) System.err.println("processArg cannot process " + arg + " as a double");
      return d;
    }
    return value;
  }
  ...
  public float processArg(String arg, float f)
  {
    ...
    {
      if (optErr) System.err.println("processArg cannot process " + arg + " as a float");
      return f;
    }
    return value;
  }
    ...
    { // handle colon, if present
      if (pos < optString.length() - 1 && optString.charAt(pos + 1) == ':')
      {
        ...
        { // take next arg as optArg
          optIndex++;
          if (optIndex < argCount && (theArgs[optIndex].charAt(0) != '-' || theArgs[optIndex].length() >= 2 && (optString.indexOf(
              theArgs[optIndex].charAt(1)) == -1 || theArgs[optIndex].charAt(1) == ':')))
          {
  ...
  {  // test the class
    ...
    while ((ch = go.getopt()) != go.optEOF)
    {
      ...
      else if ((char) ch == 'b') bflg = go.processArg(go.optArgGet(), bflg);
      else if ((char) ch == 'f') filename = go.optArgGet();
      else if ((char) ch == 'h') height = go.processArg(go.optArgGet(), height);
      else if ((char) ch == 'w') width = go.processArg(go.optArgGet(), width);
    ...
    System.out.println("These are all the command line arguments " + "before processing with lrproc.examples.GetOpt:");
