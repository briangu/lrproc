...
{
  Logger log = org.apache.log4j.Logger.getLogger(NPointCrossover.class);
  ...
  public void crossover(Chromosome x, Chromosome y)
  {
    ...
    Collections.sort(crossArray);

    if (log.isTraceEnabled())
    {
      log.trace("Npoint crossover at:");
      for (int i = 0; i < crossArray.length; i++)
      {
        log.trace(" " + crossArray[i]);
      }
    }
